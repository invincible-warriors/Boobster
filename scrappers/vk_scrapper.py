...
def get_urls_soup(path: str) -> list[str]:
    """
...
def save_info(func):
    def inner(*args, **kwargs):
        ...
        print(
            f"\n[{config.ColoredSymbols.info}] {info_text}\n"
            f"[{config.ColoredSymbols.vertical}] A total of "
            f"{config.Colors.blue}{total_added_photos_number}{config.Colors.white} photos were added\n"
            f"[{config.ColoredSymbols.vertical}] By categories:\n"
            f"[{config.ColoredSymbols.vertical}] - {total_added_photos_number_counter_formatted}\n\n"
        )
...
def save_photos(urls: [str], path: str) -> None:
    ...
    formatted_url = f" {global_url} ".center(config.console_width - 4, "-")
    print(f"[{config.ColoredSymbols.info}] {formatted_url}")
        ...
        for url in urls:
            ...
            try:
                if path in [
                    ...
                    add_photo_url(url)
                print(
                    f"[{config.ColoredSymbols.add}] {config.Colors.green}Successfully added{config.Colors.white} {short_url}"
                )

            except Exception as e:
                print(
                    f"[{config.ColoredSymbols.warn}] {config.Colors.red}{e}{config.Colors.white} {short_url}"
                )
